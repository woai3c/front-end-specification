...
当团队成员要开发新功能或改 BUG 时，需要从 dev 分支开一个新的分支：
1. 新功能分支命名格式 `feature-功能名称-开发姓名`，示例 `feature-login-tanguangzhi`。
2. BUG 分支全名格式 `fix-功能名称-itest 的 bug号`，示例 `fix-login-404`。

在将一个新分支合并回 dev 分支之前，需要查看自己的新分支中有没有多个重复提交或意义不明的 commit。如果有，则需要对它们进行合并（git rebase）或修改（git commit --amend）。示例：
```bash
# 这两个 commit 可以合并成一个
chore: 修改按钮文字
chore: 修改按钮样式
...
chore: 修改按钮样式及文字
```
最后，还需要将 dev 分支合并（merge）到当前分支，然后才可以将当前分支合并到 dev 分支。
